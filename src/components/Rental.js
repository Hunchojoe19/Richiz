...
const Rental = ({ title, image, price, index }) => {
  const navigate = useNavigate();

  const handleClick = (id) => {
    navigate(`/${id}`, { state: { title, image, price } });
  };
  return (
    <div className="">
      <div className="relative cursor-pointer">
        {/* <div className="grad absolute w-full h-full rounded-b-[1.3rem]"></div> */}
        <div className="flex" onClick={() => handleClick(index)}>
          {/* Background */}
          <img
            src={image}
            alt=""
      ...
      {/* Description */}
      <div className="pt-3 flex justify-between items-start cursor-pointer">
        {/* Left */}
        <div className="">
